...
## 2026-06-07 設計備忘:未來主題化（theme switching）

**可行**,因為樣式來源已集中在 `plot/style.py` + `ColorManager`。要讓人能切主題
(例如切回 vanilla matplotlib),走 **Theme 物件**路線,不是純 rcParams。

### 兩層要分開
- **語意色盤(semantic)**:Missense=灰、C>T=紅、AMP=salmon… 領域慣例,**換主題不變**。
- **美學層(aesthetic)**:frameless/格線、字級、accent 單色、sequential/diverging cmap、
  legend 卡片 vs inline。**這層才是主題切換的對象。**

### 設計
- `style.py` 加 `@dataclass Theme`(frameless / grid / accent / sequential_cmap /
  diverging_cmap / base_fontsize / card_legends)+ presets `HOUSE`、`MATPLOTLIB`。
- module 級 active theme:`use_theme(t)` / `current()` / `with theme(t):`。
- `style_axes` 讀 `current()`:`MATPLOTLIB` 主題下 despine=False → 近乎 noop,框線回預設。
- accent / cmap 取 `current().*` 而非 module 常數。
- `draw_legend_cards`/`fig_with_legend`:`card_legends=False` 時退回 inline `ax.legend`。

### 限制
- 純 `plt.style.use()`/mplstyle **不夠**:despine 是 per-axes、card legend / 直 colorbar
  是版面結構,不是 rcParams。
- 「完全長得像 vanilla matplotlib」需要每個結構 flag 都實作退化路徑(card legend→inline、
  直 colorbar→預設 cbar)——做得到但是額外工。

### 工作量 / 順序
- 第一步(cosmetic 可切:frameless/字級/accent/連續cmap):**小**,來源已集中。
- 第二步(legend 結構可切):中,需 inline 退化路徑。
- 現況:**不動手**,等真有換主題需求再做。語意色盤一律保留。

### 使用者自訂 YAML(可行)
Theme 是純資料(dataclass),所以可直接從 YAML 載入,零額外架構:
```yaml
# theme.yaml
frameless: false
accent: "#1f77b4"
...
base_fontsize: 11
card_legends: false
sample_label_limit: 80
```
```python
import yaml, pymaftools.plot.style as st
st.use_theme(st.Theme(**yaml.safe_load(open("theme.yaml"))))
```
- 美學層(上面那些)→ YAML 覆蓋,直觀。
- 語意色盤(mutation/TITV/functional)→ 也能做成 YAML 可覆蓋(ColorManager 讀外部
  dict),但屬選配;預設仍用內建慣例色。
- 結論:**先有 Theme dataclass,YAML 就幾乎免費**(`Theme(**dict)`)。等做 Theme 化時一併附 `Theme.from_yaml()`。
