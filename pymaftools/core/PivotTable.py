...
class PivotTable(pd.DataFrame):
    ...
    def read_sqlite(cls, db_path: str):
        ...
        sample_metadata = pd.read_sql(
            f"SELECT * FROM 'sample_metadata'", conn, index_col="sample")
        feature_metadata = pd.read_sql(
            f"SELECT * FROM 'feature_metadata'", conn, index_col="feature")
    ...
    def add_freq(self, groups: dict = {}) -> "PivotTable":
        ...
        for group in groups.keys():
            freq_data[f"{group}_freq"] = PivotTable.calculate_frequency(
                groups[group])
