...
class CancerCellFractionTable:
    ...
    def pyclone_to_sorted_table(filepath: str) -> PivotTable:
        ...
        ccf_pivot = df.pivot(
            index="mutation_id", columns="sample_id", values="cellular_prevalence"
        )
        table = PivotTable(ccf_pivot)
        table.feature_metadata["mean_ccf"] = df.groupby("mutation_id")[
            "cellular_prevalence"
        ].mean()
        table.feature_metadata["cluster"] = df.groupby("mutation_id")[
            "cluster_id"
        ].first()

        valid_clusters = table.feature_metadata["cluster"].unique()
        cluster_order = (
            table.feature_metadata.groupby("cluster")["mean_ccf"]
            .mean()
        ...
        sorted_mutation_id = (
            table.feature_metadata.reset_index()
            .set_index("cluster")
            .loc[cluster_order]
        ...
        cluster_text = ["major"] + [
            f"minor{i + 1}" for i in range(len(cluster_order) - 1)
        ]
        table_sorted.feature_metadata["cluster_text"] = table_sorted.feature_metadata[
            "cluster"
        ].map(dict(zip(cluster_order, cluster_text)))
        return table_sorted
