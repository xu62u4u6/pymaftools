...
class LollipopPlot(BasePlot):
    ...
    def __init__(
        ...
        self.domain_top_y = (
            self.config["domain_bottom_y"] + self.config["domain_height"]
        )
        self.min_lollipop_candy_y = (
            self.domain_top_y + self.config["min_lollipop_candy_y_offset"]
        )
        ...
        self.color_manager.add_cmap(
            "lollipop_mutations", self.color_manager.get_cmap("nonsynonymous")
        )
    ...
    def _generate_domain_colors(self):
        ...
        if not self.domains:
            return {}
        unique_domain_labels = sorted(
            list(set(d.get("Label", "Unknown") for d in self.domains))
        )
        ...
        try:
            ...
            if "Set2" in cmap_name and num_labels > 9:
                print(
                    f"Warning: '{cmap_name}' might not have enough distinct colors for {num_labels} domains. Consider 'tab10' or 'tab20'."
                )
            if num_labels == 0:
                return {}
    ...
    def _setup_plot(self, ax=None, fig=None):
        ...
        self.ax_main.set_ylim(
            self.config["domain_bottom_y"] - 0.1,
            self.min_lollipop_candy_y
            + self.config["max_additional_scaled_height"]
            + 0.1,
        )
        self.ax_main.set_xlabel("Amino Acid Position", fontsize=12)
        self.ax_main.set_title(
            f"Protein Domains and Mutations for {self.protein_name}",
            fontsize=16,
            pad=20,
        )
    ...
    def _plot_backbone(self):
        ...
        if self.ax_main is None:
            return
        self.ax_main.plot(
            [0, self.protein_length],
            [self.config["backbone_y"], self.config["backbone_y"]],
            color="black",
            linewidth=6,
    ...
    def _plot_domains(self):
        ...
        for domain in self.domains:
            ...
            rect = patches.Rectangle(
                (start, self.config["domain_bottom_y"]),
                end - start,
    ...
    def _plot_mutations(self):
        ...
        for mutation in self.mutations:
            ...
            mutation_color = self.color_manager.get_cmap("lollipop_mutations").get(
                m_type, "#CCCCCC"
            )

            # Calculate lollipop top Y position
            scaled_height_addition = (count / self.max_mutation_count) * self.config[
                "max_additional_scaled_height"
            ]
            ...
            self.ax_main.plot(
                [pos, pos],
                [self.config["backbone_y"], lollipop_top_y],
                color="grey",
                linewidth=1,
                zorder=0,
            )
            ...
            marker_size = (
                50 + (count / self.max_mutation_count) * 100
            )  # Optional scaling
    ...
    def plot(self, ax=None, fig=None):
        ...
        if hasattr(self, "ax_legend") and self.ax_legend is not None:
            ...
            try:
                ...
                print(
                    f"Warning: tight_layout failed. Legends might overlap or be cut off. Error: {e}"
                )
    ...
    def plot_multi_cohort(
        ...
        main_plot.fig = fig
        main_plot.ax_legend = fig.add_subplot(
            gs[:, 1]
        )  # Legend spans all rows, right column
        ...
        for i, (cohort_name, cohort_data) in enumerate(cohorts_data.items()):
            ...
            for domain in domains_data:
                ...
                if label in cohort_plot.domain_colors:
                    # Use custom label if available, otherwise use original label
                    display_label = (
                        domain_label_map.get(label, label)
                        if domain_label_map
                        else label
                    )
            ...
            for m_type in mutation_types:
                if m_type in cohort_plot.color_manager.get_cmap("lollipop_mutations"):
                    # Use custom label if available, otherwise use original mutation type
                    display_label = (
                        mutation_label_map.get(m_type, m_type)
                        if mutation_label_map
                        else m_type
                    )
                    all_mutation_colors[display_label] = (
                        cohort_plot.color_manager.get_cmap("lollipop_mutations")[m_type]
                    )
        ...
        if main_plot.ax_legend is not None:
            main_plot.plot_all_legends(
                ax=main_plot.ax_legend,
                fontsize=10,
                title_fontsize=12,
                legend_spacing=0.2,
                item_spacing=0.04,
