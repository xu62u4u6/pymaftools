"""
PivotStatsPlot Module

Statistical / exploratory plots for PivotTable objects (PCA, boxplots with
annotations, ...). Named to make clear this is *not* the base for all PivotTable
plotting — the composite oncoplot lives in OncoPlot. Kept separate from the core
PivotTable class.

``PivotTablePlot`` remains importable as a backward-compatible alias (v0.5.0).
"""
...
class PivotStatsPlot(BasePlot):
    """
    Statistical / exploratory plots for PivotTable objects.

    Provides one-shot statistical visualizations (PCA, annotated boxplots) plus
    an :meth:`oncoplot` entry point. Inherits from BasePlot for legend management
    and figure saving.
    ...
    def __init__(self, pivot_table):
        """
        Initialize PivotStatsPlot with a PivotTable instance.
    ...
    def oncoplot(self, **kwargs):
        """Create an :class:`~pymaftools.plot.OncoPlot.OncoPlot` for this table.

        Unifies the entry point so the composite oncoplot is reachable from the
        same ``table.plot`` accessor as the statistical plots, e.g.::

            table.plot.oncoplot(figsize=(15, 10)).main().add_freq().render()

        ``**kwargs`` are forwarded to ``OncoPlot`` (e.g. ``figsize``, ``cmap``).
        """
