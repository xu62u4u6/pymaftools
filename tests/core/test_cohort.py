...
class TestCohortValidation:
    ...
    def test_mismatched_samples(self, sample_pivot_table):
        ...
        # Mismatches must fail closed instead of silently dropping samples.
        with pytest.raises(ValueError, match="do not exactly match"):
            cohort.add_table(different_table, "table2")

    def test_extra_samples_are_not_silently_dropped(self, sample_pivot_table):
        cohort = Cohort("test")
        cohort.add_table(sample_pivot_table, "table1")
        ...
        extra = PivotTable(
            pd.DataFrame(
                [[True] * 5],
                index=["TP53"],
                columns=[*sample_pivot_table.columns, "extra_sample"],
            )
        )
        with pytest.raises(ValueError, match=r"extra=\['extra_sample'\]"):
            cohort.add_table(extra, "table2")
